...
%% Plot 2D Histogram of npresses vs Stimulus strength
figure
% ylim([2 14])
% xlim([0 0.7])
[N,Xbins,Ybins] = hist2d(difficulty,npresses,15,'tile');
hold on
colorbar()
set(gca,'YDir','normal')
xlabel('(NR-NL)/(NR+NL)')
...
% ylim([2 14])
% xlim([0 0.7])
[N,Xbins,Ybins] = hist2d(abs(difficulty),npresses,15,'tile');
hold on
Xbins;
Ybins;
N = flip(N);
...
figure
[N,Xbins,Ybins] = hist2d(abs(difficulty),npresses,'tile');
figure
%N = flip(N);
