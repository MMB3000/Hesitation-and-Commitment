...
%% Transform to Belief
  ...
  randomnext= nan(nstim,length(tvec)-1);
...
for dim2 = length(n_rectxs)%1:length(n_rectxs) 
...
dt = .25; %<<<<<<<<<<<<<<<<<<<<<<<< is this in general = (chunk_duration_msec/1000)?
rng(1);
...
% end
% %  legend(h,['\beta = ' ' '  num2str(round(st.tstat.beta(2)*100)/100),'p=' num2str(round(st.tstat.pval(2)*10000)/10000)],'color', [t_cost/(max(t_costs)+t_cost) t_cost/(max(t_costs)+t_cost) t_cost/(max(t_costs)+t_cost)]);
% 
% xlabel('|(NR-NL)/(NR+NL)|')
...
h =    plot((difficulty),z,'.','markersize',15, 'color', [t_cost/(max(t_costs)+t_cost) t_cost/(max(t_costs)+t_cost) t_cost/(max(t_costs)+t_cost)],'DisplayName',[ ['y' num2str(1/(t_cost*n_recty))] ' \beta = ' ' '  num2str(round(st(2)*100)/100)]);
...
%h = sort(h);
for j=1:length(h)
  saveas(h(j), ['AllAgenty_arena' num2str(n_recty) '_tc_'   num2str(t_cost) '_' 'xc_' num2str(x_cost) '_' num2str(j) 'alpha_' num2str(alpha) '.fig']);
  print(h(j), ['AllAgentny_arena' num2str(n_recty) '_tc_'   num2str(t_cost) '_' 'xc_' num2str(x_cost) '_' num2str(j) 'alpha_' num2str(alpha) '.pdf'], '-fillpage', '-dpdf');
  saveas(h(j), ['AllAgentny_arena' num2str(n_recty) '_tc_'  num2str(t_cost) '_' 'xc_' num2str(x_cost) '_' num2str(j) 'alpha_' num2str(alpha) '.png']);
end
  save(['AllAgentny_arenaTrajs' num2str(n_recty) '_tc_'  num2str(t_cost) '_' 'xc_' num2str(x_cost) '_' num2str(j) 'alpha_' num2str(alpha) '.mat'],'x','stim','stim2','stimuli','p','Bl','Br');
